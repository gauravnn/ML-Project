	...
	def optimizeMin(self, iterator = 10000):
		...
		for it in range(iterator):
			...
			for nm in range(self.m):
				#Random archive selection
				so = 500 - self.T['fitness']
				s = sum(so)

				# should be 
				# prob = [(self.T['fitness'][i]/s) for i in range(self.nAnt)]
				# j = np.random.choice(range(self.nAnt),1,prob) [But not available in numpy ver < 1.7.0 , currently not compatible with scipy)
				...
				for i in range(self.dim):
					#print self._SD(i,j), (np.exp(-100 * (iterator - globalTemp)/ iterator) * (self.limit[i][1] - self.limit[i][0])/2 ),
					res = np.random.normal(self._mean(i,j), self._SD(i,j) * (np.exp(-(iterator - globalTemp)/ iterator))) 
					#print res
					#if( globalTemp/iterator < np.random.random() ):
					#	res += (np.random.random() * (self.limit[i][1] - self.limit[i][0]) + self.limit[i][0])/2
					#else:
						
					#factor = ((self.limit[i][1] - self.limit[i][0]) / iterator * res) + self.limit[i][0]
					
					#res = self._gaussianKernel(i, res)
					#print res
